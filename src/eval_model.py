...
from graph_vae import GraphVAE, load_model_checkpoint,GraphEncoder,GRANDecoder
...
decoder = GRANDecoder(node_feature_dim, latent_dim,latent_dim,max_nodes).to(device)
...
N = 10  # Number of graphs to sample

erdos = ErdosRenyiSampler(dataset)  # Adjust p as needed

def sample_num_nodes(num_graphs, node_count_distribution, unique_node_counts):
    """Sample a list of integers representing the number of nodes in each graph.

    Parameters
    ----------
    num_graphs : int
        Number of graphs to sample.
    node_count_distribution : torch.tensor
        Empirical distribution of the number of nodes in the dataset.
    unique_node_counts : torch.tensor
        Unique node counts in the dataset.

    Returns
    -------
    list
        List of integers representing the number of nodes in each graph.
    """
    sampled_indices = torch.multinomial(node_count_distribution, num_graphs, replacement=True)
    return unique_node_counts[sampled_indices].tolist()

def compute_generated_degree_hist(batch: Batch, max_deg: int):
    # batch.x has shape (total_nodes, feat_dim)
    total_nodes = batch.x.size(0)

    # edge_index[0] holds source nodes of each edge
    deg = degree(batch.edge_index[0], num_nodes=total_nodes, dtype=torch.long)
    hist = torch.bincount(deg, minlength=max_deg + 1).float()
    return hist / total_nodes  # p_gen[k]
...
def compute_metrics(sampled_graphs, training_graphs):
    sampled_graphs_nx = [to_networkx(graph) for graph in sampled_graphs]
    training_graphs_nx = [to_networkx(graph) for graph in training_graphs]
...
num_nodes = torch.tensor(sample_num_nodes(N, node_count_distribution, unique_node_counts))


sampled_graphs = vae.sample(N,num_nodes).to_data_list() #[generator(z[i:i+1], [num_nodes[i]]) for i in range(N)]
print(f"Sampled {len(sampled_graphs)} graphs")
# Create a collage of 5x5 sampled graphs
fig, axes = plt.subplots(3, 3, figsize=(15, 15))
for i, ax in enumerate(axes.flatten()):
    if i < len(sampled_graphs):
        graph_nx = to_networkx(sampled_graphs[i])
